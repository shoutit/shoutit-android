...
public class PagesSuggestionPresenter extends BaseProfileListPresenter {
    ...
    public PagesSuggestionPresenter(@Nonnull ProfilesDao dao,
        ...
        super(listeningHalfPresenter, uiScheduler, resources.getString(R.string.invite_no_suggested_pages), userPreferences);

        final Observable<ProfilesDao.FriendsSuggestionPointer> pointerObservable = userPreferences
                .getPageOrUserObservable()
                .filter(Functions1.isNotNull())
                .map(baseProfile -> new ProfilesDao.FriendsSuggestionPointer(
                        RequestsConstants.PAGE_SIZE, baseProfile.getLocation(), baseProfile.getUsername()));
